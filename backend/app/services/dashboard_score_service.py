...
class DashboardScoreService:
    ...
    def _title(self, status: str) -> str:
        return {
            ...
            "top": "Вы почти у цели! Максимальная выгода в ваших руках",
        }[status]
    ...
    def _description(self, status: str) -> str:
        return {
            ...
            "growing": "Вы уже эффективно используете бонусы, но остаётся потенциал в программах, офферах и экосистеме.",
            "strong": "У вас стабильная история выплат, несколько программ лояльности и подходящие предложения партнёров. Так держать!",
            "top": "Вы активно используете лояльность и прогнозы, а значит экосистемные рекомендации имеют высокую актуальность!",
        }[status]
